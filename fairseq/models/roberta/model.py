...
class RobertaModel(FairseqEncoderModel):
    ...
    def add_args(parser):
        ...
        parser.add_argument(
            ...
            action="store_true",
            help="restore masked embeddings for encoder",
        )
        parser.add_argument(
            "--refill-mask",
            action="store_true",
            help="refill mask embeddings to mask positions before decoder",
        )
...
class RobertaEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary, mask_idx):
        ...
        self.refill_mask = args.refill_mask
        ...
        if args.decoder_layers > 0:
            ...
            if args.refill_mask:
                self.decoder_position_emb = nn.Embedding(args.max_positions, args.encoder_embed_dim)
        else:
            self.decoder = None
    ...
    def forward(
        ...
        **unused
        ...
        x, extra = self.extract_features(
            src_tokens, return_all_hiddens=return_all_hiddens, attn_mask=unused.get("attn_mask", None), masked_tokens=masked_tokens,
        )  
        if not features_only:
            if self.decoder is not None:
                padding_mask = src_tokens.eq(self.sentence_encoder.padding_idx)
                if self.refill_mask:
                    x[masked_tokens] = self.sentence_encoder.embed_tokens.weight[self.mask_idx]
                    pos_expand = self.decoder_position_emb.weight.unsqueeze(0).expand(x.size(0), -1, -1)[:, :x.size(1), :]
                    x[masked_tokens] = x[masked_tokens] + pos_expand[masked_tokens]
...
def base_architecture(args):
    ...
    args.refill_mask = getattr(args, "refill_mask", False)
