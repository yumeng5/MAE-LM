...
class Trainer(object):
        ...
        if bexists:
            ...
            try:
                if load_teacher_weights:
                    print(f"Loading teacher weights")
                    teacher_states = {}
                    for key in state["model"].keys():
                        # if "sentence_encoder." in key:
                        #     new_key = key.replace("encoder", "teacher_encoder", 1)
                        #     teacher_states[new_key] = state["model"][key]
                        if "small_generator." in key:
                            teacher_states[key] = state["model"][key]
                    self.model.load_state_dict(teacher_states, strict=False)
                    return None
                else:
            ...
            with torch.autograd.profiler.record_function("clip-grads"):
                ...
                for name, param in self.model.named_parameters():
                    ...
                    if param.grad is None:
                        continue
                    if name.startswith("encoder.sentence_encoder.layers"):
                        if sample_size > 0:
                            layer_grads[name.split('.')[3]].append(param.grad.float())
                    # if name.startswith("encoder.lm_head"):
                    #     if sample_size > 0:
                    #         lm_head_grads.append(param.grad.float())
                    if name.startswith("encoder.sentence_encoder.embed_tokens.weight"):
                        if sample_size > 0:
                            token_emb_grads.append(param.grad.float())
                # if len(lm_head_grads) > 0:
                #     lm_head_grad = torch.norm(torch.stack([torch.norm(p.detach()) for p in lm_head_grads]))
                #     log_val = self.optimizer.scale_grad_norm(lm_head_grad.item())
                #     metrics.log_scalar(f'gnorm_lm_head', log_val, priority=400, round=3, weight=0)
                if len(token_emb_grads) > 0:
                    emb_grad = torch.norm(torch.stack([torch.norm(p.detach()) for p in token_emb_grads]))
                    log_val = self.optimizer.scale_grad_norm(emb_grad.item())
                    metrics.log_scalar(f'gnorm_token_emb', log_val, priority=400, round=3, weight=0)
                for i in layer_grads:
                    layer_grad = torch.norm(torch.stack([torch.norm(p.detach()) for p in layer_grads[i]]))
                    log_val = self.optimizer.scale_grad_norm(layer_grad.item())
                    metrics.log_scalar(f'gnorm_layer_{i}', log_val, priority=400, round=3, weight=0)
    ...
    def _reduce_and_log_stats(self, logging_outputs, sample_size, grad_norm=None):
        if grad_norm is not None and (
            ...
            metrics.log_scalar("gnorm", grad_norm, priority=400, round=3, weight=0)
