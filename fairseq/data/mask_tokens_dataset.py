...
class MaskTokensRemoveDataset(BaseWrapperDataset):
    """
    A wrapper Dataset for masked language modeling.
    ...
    def apply_mask(cls, dataset: torch.utils.data.Dataset, *args, **kwargs):
        ...
        dataset = LRUCacheDataset(dataset)
        return (
            LRUCacheDataset(cls(dataset, *args, **kwargs, return_masked_tokens="tokens")),
            LRUCacheDataset(cls(dataset, *args, **kwargs, return_masked_tokens="targets")),
            LRUCacheDataset(cls(dataset, *args, **kwargs, return_masked_tokens="pos")),
        )
    ...
    def __getitem_cached__(self, seed: int, epoch: int, index: int):
        with data_utils.numpy_seed(self.seed, self.epoch, index):
            ...
            if self.mask_prob == 0:
                # exit early
                if self.return_masked_tokens == "tokens" or self.return_masked_tokens == "masks":
                    new_item = np.copy(item)
                elif self.return_masked_tokens == "targets":
                    new_item = np.full(len(item), self.mask_idx)
                else:
                    new_item = np.arange(sz)
                return torch.from_numpy(new_item)
            ...
            if self.return_masked_tokens == "targets":
                # exit early if we're just returning the masked tokens
                # (i.e., the targets for masked LM training)
                if self.mask_whole_words is not None:
                    mask = np.repeat(mask, word_lens)
                new_item = np.full(len(mask), self.mask_idx)
                new_item[mask] = item[torch.from_numpy(mask.astype(np.uint8)) == 1]
                return torch.from_numpy(new_item)
            elif self.return_masked_tokens == "pos":
                num_pos_shift = 0
                if self.pos_shift > 0 and np.random.rand() < 0.5:
                    num_pos_shift = np.random.randint(0, self.pos_shift)
                new_item = np.arange(len(mask)) + num_pos_shift
