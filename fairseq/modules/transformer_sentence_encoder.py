...
class TransformerSentenceEncoder(nn.Module):
    ...
    def get_rel_pos_bias(self, x, pos_idx=None):
        if self.rp_bucket.device != x.device:
            self.rp_bucket = self.rp_bucket.to(x.device)
        if pos_idx is not None:
            pos_idx_1 = pos_idx.unsqueeze(1).repeat(1, self.rp_bucket.size(0), 1)
            rp_bucket_1 = self.rp_bucket.unsqueeze(0).repeat(pos_idx.size(0), 1, 1).gather(-1, pos_idx_1)
            pos_idx_2 = pos_idx.unsqueeze(-1).repeat(1, 1, rp_bucket_1.size(-1))
            rp_bucket = rp_bucket_1.gather(1, pos_idx_2)
            ...
            values = values.repeat(x.size(0), 1, 1)
        return values

    def shift_position_emb(self, position_shift):
        position_embs = self.embed_positions.weight.unsqueeze(0).expand(position_shift.size(0), -1, -1)
        return position_embs[torch.arange(position_shift.shape[0])[:, None], position_shift]
    ...
    def forward(
        ...
        position_shift: Optional[torch.Tensor] = None,
        token_embeddings: Optional[torch.Tensor] = None,
        attn_mask: Optional[torch.Tensor] = None,
        use_ext_padding_mask: bool = False,
        padding_mask: Optional[torch.Tensor] = None,
        restore_pos: Optional[torch.Tensor] = None,
        **unused,
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        is_tpu = False #tokens.device.type == "xla"
        assert tokens is not None or token_embeddings is not None
        ...
        seq_len = tokens.size(1) if tokens is not None else token_embeddings.size(1)
        if self.embed_positions is not None:
            if position_index is not None:
                weight = self.embed_positions(position_index)
            elif position_shift is not None:
                weight = self.shift_position_emb(position_shift)
            else:
        ...
        x_tokens = tokens if tokens is not None else token_embeddings
        rel_pos_bias = self.get_rel_pos_bias(x_tokens, position_index) if self.rel_pos else None
        if attn_mask is None:
            attn_mask = rel_pos_bias
            attn_mask_all = None
        else:
            attn_mask_val = torch.zeros(attn_mask.size(), device=attn_mask.device, dtype=x.dtype)
            attn_mask_val.masked_fill_(attn_mask, float("-inf"))
            attn_mask_val = attn_mask_val.unsqueeze(1)
            if padding_mask is not None:
                attn_mask_val.masked_fill_(
                    padding_mask.unsqueeze(1).unsqueeze(2).to(torch.bool),
                    float("-inf")
                )
                padding_mask = None
            attn_mask_all = attn_mask_val.repeat(1, self.num_attention_heads, 1, 1)
            attn_mask = attn_mask_all.view(-1, seq_len, seq_len)
            if rel_pos_bias is not None:
                attn_mask = attn_mask + rel_pos_bias
        # if attn_mask is not None and padding_mask is not None:
        #     # merge key_padding_mask and attn_mask
        #     # don't merge two mask if using gru_rel_pos
        #     attn_mask = attn_mask.view(attn_mask.size(0), tokens.size(0), -1, seq_len, seq_len)
