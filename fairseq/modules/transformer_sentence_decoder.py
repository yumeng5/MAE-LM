...
class TransformerSentenceDecoder(nn.Module):
    """
    ...
    def __init__(
        ...
        self.layers.extend(
            [
                self.build_transformer_sentence_decoder_layer(
                    ...
                    first_cross_attn=first_cross_attn,
                    cross_self_attention=cross_self_attention,
                )
    ...
    def build_transformer_sentence_decoder_layer(
        self,
        embedding_dim,
        ...
        layer = TransformerSentenceDecoderLayer(
    ...
    def forward(
        ...
        elif type(token_embeddings) == torch.Tensor:
            seq_len = token_embeddings.size(1)
        else:
            seq_len = position_index.size(1)
        ...
        self_attn_mask = None if full_context_alignment else self.buffered_future_mask(x)
        if rel_pos_bias is not None:
            self_attn_mask = self_attn_mask + rel_pos_bias if self_attn_mask is not None else rel_pos_bias
        for i, layer in enumerate(self.layers):
            x, _ = layer(x, encoder_out, self_attn_padding_mask=padding_mask, self_attn_mask=self_attn_mask, encoder_padding_mask=encoder_padding_mask)
            if not last_state_only:
                inner_states.append(x)
